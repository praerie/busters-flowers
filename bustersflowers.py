...
def create_flower(flower_type=SUNFLOWER, base_petal_count=21):
    """
    ...
    for layer_type, petal_count in zip(layer_types, petal_counts):
        ...
        all_petals.extend(arranged_layer)
...
def create_petal(layer_type, flower_type):
    ...
    petal = cmds.polyCube(
        w=flower_type['width'], h=flower_type['height'], d=flower_type['depth'], 
        sx=flower_type['subdiv-x'], sy=flower_type['subdiv-y'], name='petal'
    )[0]
    # To-do: Fix bottom/back side of petal shape

    # Save petal vertices to list
    num_vertices = cmds.polyEvaluate(petal, vertex=True)

    # Draw petal curvature based on flower type
    move_vertices(petal, num_vertices)
    # To-do: Auto-adjust based on flower type?

    return petal


def move_vertices(petal, num_vertices):
    """
    ...
    left_vertices, right_vertices = get_vertex_ranges(num_vertices)

    # Create petal curvature on left
    for i, vertex in enumerate(left_vertices):
        pos = PETAL_VERTEX_MOVES[i % len(PETAL_VERTEX_MOVES)]
...
def get_vertex_ranges(vertex_count):
    """
...
def transform_petal(petal, flower_type, layer):
    """
    ...
    # To-do: Scale and tilt around disk, accounting for flower_type
...
def arrange_petals(petal, petal_count, flower_type, layer):
    ...
    for i in range(petal_count):
        ...
        transform_petal(petal_instance, flower_type, layer)
